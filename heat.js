...
async function fetchCodeforcesSubmissions(user) {
    try {
        ...
        if (data.status !== 'OK') {
            console.warn(`Codeforces API error for ${user}: ${data.comment || 'Unknown error'}`);
            return [];
        }
...
function createUserCard(userName, atcoderHandle, codeforcesHandle, submissions, year) {
...
function showTooltip(event, date, count) {
    ...
    tooltip.textContent = `${date}: ${count} submission${count !== 1 ? 's' : ''}`;
...
function calculateStats(submissions, year) {
    ...
    sortedDates.forEach((date, index) => {
        if (lastDate) {
            const daysDiff = (new Date(date) - new Date(lastDate)) / (1000 * 60 * 60 * 24);
            if (daysDiff === 1) {
                tempStreak++;
            } else {
        ...
        if (index === sortedDates.length - 1) {
            const daysFromToday = (new Date(today) - new Date(date)) / (1000 * 60 * 60 * 24);
            if (daysFromToday <= 1) {
...
async function loadAndRenderHeatMaps() {
    ...
    try {
        ...
        document.getElementById('error-message').textContent = err.message || 'Failed to load heat map data.';
    }
}
...
// Update loading progress message with user name and percentage
...
//             progressText.innerHTML = `<strong>Initializing...</strong><br>Preparing to load ${total} users`;
//         } else if (userName === 'Complete!') {
//             progressText.innerHTML = `<strong style="color: var(--secondary-color);">✓ Loading Complete!</strong><br>${total} / ${total} users loaded (100%)`;
//         } else {
//             progressText.innerHTML = `Loading data for <strong>${userName}</strong><br>${loaded} / ${total} users (${percentage}%)`;
//         }
//     }
// }
...
document.getElementById('show-atcoder').addEventListener('change', renderHeatMaps);
document.getElementById('show-codeforces').addEventListener('change', renderHeatMaps);

// Retry button
document.getElementById('retry-btn')?.addEventListener('click', loadAndRenderHeatMaps);
